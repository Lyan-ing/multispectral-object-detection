...
def img2label_paths(img_paths, images_folder='images'):
    # Define label paths as a function of image paths
    sa, sb = os.sep + images_folder + os.sep, os.sep + 'labels' + os.sep  # /images/, /labels/ substrings
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # Check cache
        self.label_files = img2label_paths(self.img_files, 'visible')  # labels
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # Check cache
        # Check rgb cache
        rgb_name = path_rgb.split('/')[-2]
        self.label_files_rgb = img2label_paths(self.img_files_rgb, rgb_name)  # labels
        # print(self.label_files)
        cache_rgb_path = str(p_rgb if p_rgb.is_file() else Path(self.label_files_rgb[0]).parent)+(f'_{rgb_name}.cache')  # cached labels
        cache_rgb_path = Path(cache_rgb_path)
        ...
        # Check ir cache
        ir_name = path_ir.split('/')[-2]
        self.label_files_ir = img2label_paths(self.img_files_ir, ir_name)  # labels
        # print(self.label_files)
        cache_ir_path = str(p_ir if p_ir.is_file() else Path(self.label_files_ir[0]).parent)+(f'_{ir_name}.cache')  # cached labels
        cache_ir_path = Path(cache_ir_path)
        ...
        bi_rgb = np.floor(np.arange(n_rgb) / batch_size).astype(np.int16)  # batch index
        ...
        bi_ir = np.floor(np.arange(n_ir) / batch_size).astype(np.int16)  # batch index
        ...
        if self.rect:
            ...
            self.batch_shapes_rgb = np.ceil(np.array(shapes_rgb) * img_size / stride + pad).astype(np.int16) * stride
            ...
            self.batch_shapes_ir = np.ceil(np.array(shapes_ir) * img_size / stride + pad).astype(np.int16) * stride
